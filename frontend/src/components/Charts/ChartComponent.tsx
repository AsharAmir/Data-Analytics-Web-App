...
interface ChartComponentProps {
  ...
  description?: string;
  loading?: boolean;
  onDataPointClick?: (datasetIndex: number, index: number, value: number) => void;
  ...
  const exportChart = (format: 'png' | 'pdf') => {
    ...
    if (chart) {
      ...
      if (format === 'png') {
        ...
        pdf.text('Generated by Financial Analytics Dashboard', margin, pageHeight - 10);
        
        pdf.save(`${(title || 'chart').replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().getTime()}.pdf`);
      }
    }
    onExport?.(format);
  };
