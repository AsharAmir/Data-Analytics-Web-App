...
const DataExplorerPage: React.FC = () => {
  ...
  const [sqlQuery, setSqlQuery] = useState('SELECT * FROM SAMPLE_BT WHERE ROWNUM <= 100');
  ...
  const sampleQueries = [
    ...
    },
    {
      ...
      query: 'SELECT CT_PRINACT, COUNT(*) as count, AVG(FCC_ION) as avg_ion FROM SAMPLE_BT GROUP BY CT_PRINACT HAVING COUNT(*) > 5'
    },
    {
      name: 'Risk Analysis',
      query: 'SELECT BT_AS_PRAP_IN_RO, SUM(FCC_BKV) as total_value FROM SAMPLE_BT GROUP BY BT_AS_PRAP_IN_RO ORDER BY total_value DESC'
    },
    {
      name: 'Daily Summary',
      query: 'SELECT DAY_OF, CT_MAIN, SUM(FCC_BKV) as total_bkv FROM SAMPLE_BT GROUP BY DAY_OF, CT_MAIN ORDER BY DAY_OF, CT_MAIN'
    }
  ];
          ...
          <div className="px-6 py-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-600">Interactive data analysis and exploration</p>
              </div>
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 p-6 space-y-6">
          ...
          {queryResult && (
            <div className="space-y-6">
              {queryResult.success ? (
                <>
                  {viewMode === 'table' && queryResult.data && 'columns' in queryResult.data ? (
                    <DataTable
                    ...
                    />
                  ) : viewMode === 'chart' && queryResult.data && 'columns' in queryResult.data ? (
                    <ChartComponent
                      data={transformDataForChart(queryResult.data as TableData)}
                      type={chartType}
                      title="Query Results Visualization"
                      description={`${(queryResult.data as TableData).data.length} records visualized`}
                      height={400}
                    />
                  ) : (
                ...
                <div className="bg-red-50 border border-red-200 rounded-lg p-6">
                  <div className="flex items-start">
                    ...
                    <div className="ml-3 flex-1">
                      ...
                      <p className="text-sm text-red-700 mt-1">{queryResult.error}</p>
                      
                      <div className="mt-3 p-3 bg-red-100 rounded-md">
                        <h4 className="text-sm font-medium text-red-800 mb-2">Common Solutions:</h4>
                        <ul className="text-sm text-red-700 list-disc list-inside space-y-1">
                          <li>Check column names are correct and use uppercase (DAY_OF, CT_MAIN, FCC_BKV)</li>
                          <li>Ensure table name is SAMPLE_BT (uppercase)</li>
                          <li>Try one of the sample queries above to test connectivity</li>
                      ...
                      {queryResult.execution_time && (
                        <p className="text-xs text-red-600 mt-2">
                          Query failed after {queryResult.execution_time.toFixed(2)}s
                        </p>
                      )}
                    </div>
                  </div>
